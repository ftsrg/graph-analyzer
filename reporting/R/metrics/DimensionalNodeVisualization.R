DimensionalNodeVisualization <- function(dimen.node, dt = T){
  dimen.node <- dcast.data.table(data = dimen.node, file.name + GraphType + Index  + Instance ~ Category,
                                 value.var = "Value")
  if(dt) {
...
DimensionalNodeVisualizationContainment <- function(dimen.node){
  dimen.node <- dcast.data.table(data = dimen.node, file.name + GraphType + Index  + Instance + Iscontainment ~ Category,
                                 value.var = "Value")
  dimen.type <- subset(dimen.node, DimensionalTypedClusteringCoefficientList > 0)
  PlotsHistogramByFileName(dt = dimen.type,
