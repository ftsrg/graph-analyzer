DimensionalEdgeVisualization <- function(edge.type, basic.information, figures.path){
  ...
  number.of.dims <- edge.type[,  list(NumberOfDimensions=length(Instance)), by = c("file.name", "GraphType")]
  number.of.dims <- merge(basic.information, number.of.dims, by = c("file.name", "GraphType"))
  PlotsScatterplotByFileName(number.of.dims, paste0(figures.path, "numberofdimensions.pdf"),
                     x = "NumberOfEdges", y = "NumberOfDimensions", col = "GraphType", label = "file.name")
...
#  PlotsBasicScatterplot(edge.type, paste0(figures.path, "DimensionActivityratio.pdf"), x = "EdgeDimensionConnectivity", y = "NodeToEdgeRatioByType", col = "GraphType", facetwrap = "file.name", scalex = F)
}
