...
distribut <- dcast.data.table(distribut, file.name + ModelType + Iscontainment + Instance + Index ~ Category, value.var = "Value")
...
l <- lapply(seq(6, length(distribut)), function(col.index){
  ...
  tmp <- distribut[,
                   c("file.name", "ModelType", "Iscontainment", "Instance", "Index", colnames(distribut)[col.index]),
                   with = F]
