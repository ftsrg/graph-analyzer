...
public class SesameModelAnalyzer extends RDFModelAnalyzer {
...
//			numberOfNodesWithOutgoingDegrees += degree > 0 ? 1 : 0;
			nodesMetric.increaseNodes();

			maximumDegreeMetric.changeMaximumDegree(EdgeDirection.OUTGOING, degree);
	...
	protected void determineClustering(final URI subject) throws RepositoryException,
		...
		clusteringCoefficientMetric.addClusteringCoefficient(connected, neighbors.size(), STATION);
	...
	protected void calculateEdges(TupleQueryResult results) throws QueryEvaluationException {
		...
		while (results.hasNext()) {
			set = results.next();
			edgesMetric.setNumberOfEdges(Double
					.parseDouble(set.getValue("triples").stringValue()));
		}
	}
