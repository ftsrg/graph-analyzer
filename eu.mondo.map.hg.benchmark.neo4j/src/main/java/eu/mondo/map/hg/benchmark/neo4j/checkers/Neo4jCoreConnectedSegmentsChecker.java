	...
	public Collection<Neo4jConnectedSegmentsMatch> check() throws IOException {
		...
		try (Transaction tx = graphDb.beginTx()) {
			...
			while (sensors.hasNext()) {
				...
				final Iterator<Relationship> sensorEdges1 = sensor.getRelationships(
						Direction.INCOMING, Neo4jConstants.relationshipTypeSensor)
						.iterator();
				while (sensorEdges1.hasNext()) {
					...
					final Iterator<Relationship> connectsTo1s = segment1
							.getRelationships(
									Direction.OUTGOING,
					...
					final Iterator<Relationship> connectsTo2s = segment2
							.getRelationships(
									Direction.OUTGOING,
									Neo4jConstants.relationshipTypeConnectsTo)
							.iterator();
					...
					final Iterator<Relationship> connectsTo3s = segment3
							.getRelationships(
									Direction.OUTGOING,
									Neo4jConstants.relationshipTypeConnectsTo)
							.iterator();
					...
					final Iterator<Relationship> connectsTo4s = segment4
							.getRelationships(
									Direction.OUTGOING,
									Neo4jConstants.relationshipTypeConnectsTo)
							.iterator();
					...
					final Iterator<Relationship> connectsTo5s = segment5
							.getRelationships(
									Direction.OUTGOING,
									Neo4jConstants.relationshipTypeConnectsTo)
							.iterator();
					...
					final Iterator<Relationship> sensorEdges2 = segment6
					...
					if (sensor.equals(sensor2)) {
						...
						final Neo4jConnectedSegmentsMatch csm = new Neo4jConnectedSegmentsMatch(
								match);
						matches.add(csm);
					}
				}
