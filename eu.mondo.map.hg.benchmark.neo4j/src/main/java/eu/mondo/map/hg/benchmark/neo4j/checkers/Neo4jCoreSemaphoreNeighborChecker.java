...
import eu.mondo.map.hg.benchmark.neo4j.constants.Neo4jConstants;
import eu.mondo.map.hg.benchmark.neo4j.driver.Neo4jDriver;
import eu.mondo.map.hg.benchmark.neo4j.matches.Neo4jSemaphoreNeighborMatch;
	...
	public Collection<Neo4jSemaphoreNeighborMatch> check() {
		...
		try (Transaction tx = graphDb.beginTx()) {
			...
			while (routes1.hasNext()) {
				...
				final Iterable<Relationship> exits = route1.getRelationships(
						Direction.OUTGOING, Neo4jConstants.relationshipTypeExit);
				for (final Relationship exit : exits) {
					...
					final Iterable<Relationship> definedBys1 = route1.getRelationships(
							Direction.OUTGOING,
					...
					for (final Relationship definedBy1 : definedBys1) {
						...
						final Iterable<Relationship> relationshipSensors1 = sensor1
								.getRelationships(
										Direction.INCOMING,
						...
						for (final Relationship relationshipSensor : relationshipSensors1) {
							...
							final Iterable<Relationship> connectsTos = te1
									.getRelationships(
											Direction.OUTGOING,
							...
							for (final Relationship connectsTo : connectsTos) {
								...
								final Iterable<Relationship> relationshipSensors2 = te2
										.getRelationships(
												Direction.OUTGOING,
												Neo4jConstants.relationshipTypeSensor);
								for (final Relationship relationshipSensor2 : relationshipSensors2) {
									final Node sensor2 = relationshipSensor2
											.getEndNode();
									...
									final Iterable<Relationship> definedBys2 = sensor2
											.getRelationships(
													Direction.INCOMING,
													Neo4jConstants.relationshipTypeDefinedBy);
									for (final Relationship definedBy2 : definedBys2) {
										final Node route2 = definedBy2
												.getStartNode();
										...
										if (route1.getId() == route2
												.getId()) {
											break;
										}

										// (route2)-[:entry]-(semaphore) NAC
										final Iterable<Relationship> entries2 = route2
												.getRelationships(
														Direction.OUTGOING,
														Neo4jConstants.relationshipTypeEntry);
										final Iterator<Relationship> entriesIterator2 = entries2
												.iterator();
										if (!entriesIterator2
												.hasNext()
												|| !entriesIterator2
											...
											match.put(VAR_SEMAPHORE,
													semaphore);
											match.put(VAR_ROUTE1,
													route1);
											match.put(VAR_ROUTE2,
													route2);
											match.put(VAR_SENSOR1,
													sensor1);
											match.put(VAR_SENSOR2,
													sensor2);
											match.put(VAR_TE1,
													te1);
											match.put(VAR_TE2,
													te2);
											matches.add(new Neo4jSemaphoreNeighborMatch(
													match));
											break;
										}
									}
