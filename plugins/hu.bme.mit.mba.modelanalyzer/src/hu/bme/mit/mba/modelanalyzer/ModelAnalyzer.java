...
public class ModelAnalyzer extends Analyzer<String, ModelMetric> {
  ...
  public ModelAnalyzer omit(ModelMetricsEnum metric) {
    ...
    if (!metrics.containsKey(metric.toString())) {
      logger.warn("The ModelAnalyzer does not contain " + metric.toString()
          + " which is planned to be omitted.");
    } else {
  ...
  public <N, T> ModelAnalyzer evaluate(final ModelAdapter<N, T> adapter) {
    for (ModelMetric m : metrics.values()) {
      m.evaluate(adapter);
    }
    return this;
  }
   ...
   *           if the adapter parameter does not implement an interface that is necessary for the
