 ...
 *******************************************************************************/
package eu.mondo.map.hg.benchmark.jena.driver;

import static eu.mondo.map.hg.rdf.RDFConstants.BASE_PREFIX;
...
import eu.mondo.map.hg.benchmark.rdf.RDFDatabaseDriver;
import eu.mondo.map.hg.constants.Query;
import eu.mondo.map.hg.rdf.RDFConstants;
...
public class JenaDriver extends RDFDatabaseDriver<Resource> {
	...
	@Override
	public List<QuerySolution> runQuery(final Query query, final String queryDefinition)
			throws IOException {
	...
	protected void deleteEdges(final Collection<Resource> vertices, final String edgeType,
			final boolean outgoing, final boolean all) {
		final Property property = model.getProperty(RDFConstants.BASE_PREFIX + edgeType);

		for (final Resource vertex : vertices) {
			final Selector selector = outgoing ? new SimpleSelector(vertex, property,
					(RDFNode) null) //
